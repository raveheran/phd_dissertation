...
% list of formulae
\newpage
\fixme{counter doesn't increase for algorithms. why?}
\addcontentsline{toc}{chapter}{List of Formulae and Algorithms}
\listofmyequations

% list of acronyms
\newpage
\fixme{some abbreviations page numbers of mentioned are not in order or repeated}
%\acresetall % reset in order not to include mentiones in non-content pages
...
\begin{tabularx}{\linewidth}{l@{\quad}X}
	$p(x \mid y,\ z)$ & the conditional probability of $x$ given the context ${y, z}$\\
	$\vec{r}$ & vector $r$ \\
	...
	$\int_{i}^{j}f(x)$ & \\ the integral over the function $f(x)$ between the points $i$ and $j$, representing the conceptual area under the curve of the function.
...
\Cref{part:introduction} introduces the background and topics necessary for addressing this intertwinement.
