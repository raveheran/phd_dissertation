...
\todo{many bracket-references for both cases}
...
More details about the dataset and the tasks are in \cref{sec:dataset_hhci}.
Investigating such a scenario contributes to the understanding both the role of an agent in interactions with multiple humans and the role of one human interlocutor on the other.
These two aspects are examined in the two \emph{components} of the study:
The \emph{addressee} component focuses on the differences between the participants' addressed interlocutor within a conversation \citep[see]{Raveh2019ESSV}, and the \emph{crowd} components spotlights the influence of an additional human interlocutor on the participants' speech toward the agent \citep[see][]{Raveh2019InterspeechAlexa}.
...
The comparisons were made based on the tasks performed by participants using the \ac{va} in both components, alternately interacting with the to the \ac{va} alone or with a confederate.
\todo{need more motivation for the two components?}

\section[The voice assistant conversation corpus]{Dataset} % \acl{vacc}
\label{sec:dataset_hhci}
...
In both condition, two interactions took place, one for each task of the performed tasks:
...
The dataset contains recordings of 27 (14 female) German native speakers in age range of 20 to 32 years (mean 24 $\pm$3.3).
...
\Cref{fig:hds_dds_dist_signif,fig:hds_dds_dist_nonsignif} shows examples of the distributions of a participant's \ac{f0} in \ac{hds} and \ac{dds} contexts.
...
\begin{table}[t]
	...
	\begin{tabularx}{\linewidth}{XSSS}
		...
		\bottomrule	
	\end{tabularx}
	\todo[inline]{combine result tables of both components}
\end{table}
...
\putref{reference to part about CAT where it says that it's a natural, unaware process}
...
Regarding the individual features, one possible explanation for the different \ac{f0} distributions is the natural difference in male and female \ac{f0} (Alexa used a female voice while the confederate was always male), which leaves a room for accommodation for participants of both sexes.
...
\todo{these two references should be mentioned 1) when presenting avatar/embodied agent types}
...
This indicates that \ac{ar} does not tend to vary as much as \ac{f0} and intensity.
...
\todo{write a few discussion sentences about temporal aspect before moving to open questions}
...
\todo{mention that the no-effect results match the no-effect results in Schweitzer2013convergence}

\todo{find a newer reference for Praat (maybe from music paper?)}