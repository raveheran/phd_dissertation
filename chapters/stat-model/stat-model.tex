...
\todo[inline]{consider a section or some free text for introduction (per seb's suggestion) where the main idea of the chapter is explained before diving into details}
...
Whereas the computational model introduced in \cref{chap:computational_model} make \aclp{sds} \emph{responsive} per the definition in \cref{subsec:accommodation_levels}, the approach here leads to a model that grants \emph{profiles} and \emph{variability}.
...
Since accommodation analyses usually refer to the \textit{difference} between values in different productions (as opposed to the values themselves), stationary kernels are more suitable for fitting \ac{gp} for them, as they are shaped by the distances between each pair of datapoints rather than their absolute values.
...
An additive kernel with constant, RBF, and noise terms is used here (see \crefrange{eq:constant_kernel}{eq:RBF_kernel} below).
...
\Cref{fig:diff_and_derivatives} demonstrates this on the same conversation from \cref{fig:gp_vacc} using the mean prediction of each speaker.
%
\begin{figure}[t]
	\centering
...
\todo{seb had a serious remark regarding this sentence}
...
The time series gradient extraction process described in \cref{sec:measuring_changes} is greatly high dimensional, even for short interactions.
...
\begin{figure}[t]
	...
	\subfigure[\acs{paa} of the original time seires.
			   The blue continuous line shows the z-normalied original time series of mutual change gradients in a conversation, which was extracted as described in \cref{sec:measuring_changes}.
...
As explained by \citet{Apostolico2003monotony}, it is preferable to use a discretization technique that uses a symbol set with equiprobability\footnote{It can be claimed that realistically equiprobability does not hold for the symbols in this kind of data.
...
To achieve the goal of the generating behaviors based on extracted core behaviors, the behavior description method from \cref{sec:measuring_changes} and the n-gram approach from \cref{sec:accommodation_as_a_lm} need to be combined.
