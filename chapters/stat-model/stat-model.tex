...
Viewing accommodation as time series helps to explore it a process evolving over time and enables a range of analysis methods.
...
Specifically, \acp{gp} are used here for \emph{kriging} (\cref{subsec:interploating_data_using_kriging}), an interpolation technique, which is a technique used in applications in various fields, like geostatistics, economics, and astronomy, to name a few.
...
One way of achieving that is using \ac{loess} smoothing (see \cref{fig:hds_dds_time_pitch}), which is adequate for gaining a smooth estimation of a speaker's overall performance.
...
A more evolved approach is presented here to describe a speaker's vocal behavior as a distribution of functions that match the accumulated \textit{evidence} from that speaker's productions.
...
In order to generate accommodation sequences, a model is needed, which can iteratively emit a label based on the seen label history. 
...
Note that the term \emph{synchrony} is avoided here for describing a steady distance between the speakers, per the definition introduced in \cref{subsec:variation_types}, which entails additional properties.
...
The size of the n-gram determines the amount of previously acquired evidence that can be used to calculate a subsequent value.
...
It cannot be expected to find a completely distinct pattern for each speaker.
...
With numeric values assigned to the degrees of changes over the course of an interaction via the \ac{paa} representations created in \cref{subsec:dim_reduction_and_symbolic_rep}, the distances between the 54 interactions can be measured and compared.
...
This process was used to simulate accommodation changes during a conversation using the three clusters in \cref{fig:knn_clustering}.
...
These models were then used to generate an accommodation response for each turn, similarly to those shown in \cref{tab:generated_symbol_sequences}, but longer and with concrete values in addition to the label.
...
All the models start with the mean value of the system in the data, which is known also in a real-world scenario from the development of the system's \ac{tts} component.
