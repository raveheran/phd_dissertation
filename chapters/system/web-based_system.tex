...
\todo[inline]{consider only mention the eE feature in the showcase, because in the validation only this one is treated}
...
\end{Verbatim}
\todo[inline]{make sure the verbatim doesn't get split by the table in the next page}
%
...
The experimental procedure stayed as faithful as possible to the procedure of the original experiment, even though more aspects of it could be automated.

The domain file created for this showcase was designed to substitute the role of the experimenter in the shadowing phase (cf.\ \cref{fig:HCIConvFlow}), i.e., presenting -- and speaking -- the next stimulus to the participant.
...
%	\vspace{-1cm}\hfill\hspace{-1cm}{\hbox{\LARGE $\Longrightarrow$}}\hfill
	...
	\todo[inline]{refer to this figure in 1-2 places (probably when talking about calculation method. say that higher convergence is likely to converge too quickly and overshoot if the user's input is not stable (as can be shown in \cref{tab:validation_baseline}))}
\end{figure}
%
...
\todo[inline]{put this in general discussion}
