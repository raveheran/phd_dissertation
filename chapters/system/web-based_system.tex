	...
	\begin{landscape}
		\begin{figure}[t]
			...
			\caption[Web system in-browser \acs{gui}]
				{Screen shot of the system's \acl{gui}, with the chat area on the top left, the interaction area on the bottom left, the plot area on the top right, and the notification area on the bottom right.
				In both the chat area and the plot area, the user is represented by the color blue and the system by orange.}
			\label{fig:gui}
		\end{figure}
	\end{landscape}
}
...
As a showcase of the system capabilities, it was utilized to replicate the shadowing experiment described in \cref{chap:shadowing_in_sung_music_and_human_computer_interaction}.
...
For the experiment simulation, two of the three features investigated in the original experiment (\cref{chap:shadowing_in_sung_music_and_human_computer_interaction}) were used.
...
%		\emph{I}   		& \emph{am} 			& \emph{addicted}				& \emph{to} 		& \emph{chocolate.} \\[0.1cm]
	...
	\caption[Example sentence for selected phonetic features]
		...
		Each stimulus contains only one feature.}
...
%%%%%%%%%%%%%
\todo[inline]{text from above, insert somewhere}
...
%	\vspace{-1cm}\hfill\hspace{-1cm}{\hbox{\LARGE $\Longrightarrow$}}\hfill
	...
	\todo[inline]{this figure is not referred to anywhere}
\end{figure}
%
...
%	\todo{combine into one table and add \% sign to upper}
%\end{table}
