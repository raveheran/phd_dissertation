...
According to \citet{Weise2017towards}, integrating these capabilities into computer systems will enhanced \ac{hci} and provide improved tools for studying accommodation also from the computers' side.
...
\todo[inline]{expand points from Oviatt}
...
For computers, however, these steps must be broken down, as they lack the common social background knowledge and the intuition as for how to match their voice to the situation.
...
A roadmap towards such systems is discussed in \cref{subsec:suggested roadmap} and a terminology describing the different levels and ways to achieve accommodative vocal behavior in machines is introduced in \cref{subsec:accommodation_levels}.
...
The social conventions and and unwritten rules of dialogue are implicitly learned by humans, who intuitively know how to apply them and when they can be altered.
This is an automatic process that is still too complex for integrating into computers.
Therefore, the way we interact with computers is often immensely different from the communication with other human beings (\cref{subsec:verbal_interaction}).
Another reason dialogue is hard is the infinitely many possible trajectories of each measured aspect.
It is always possible to create a dialogue than hasn't been produced before, even with respect to a single aspect like chosen words, floor change, vocal properties, length, or any feature of any modality.
As if language understanding isn't a hard enough task for computers, the relations between of all these aspects and their influence on one another make this task even more complex.
...
Therefore, it is assumed that substantial, maybe human-like, intelligence is required to addresses its unexpected circumstance, and no specific algorithm or \ac{ml} method is able to solve it \citep[see further reasoning and examples in][pp.\~54-57]{Shapiro1992encyclopedia}.
...
One example for this is that humans don't strictly distinct between task oriented and chitchat dialogues (\cref{tab:sds_types}), whereas modeling approaches do (although recent hybrid models have the two intertwined to some extent).
\todo{if need a reference for that, take from page 3 of Milica's slides}
...
As a process that happens as part of a dialogue, accommodation involves some of these challenges (e.g., there may always a pattern never seen before), but within a limited scope.
...
Humans might be able to point out if an interlocutor uses accommodation in a terribly wrong way (for example, continuously perfectly imitating or demonstratively attempting to talk differently), but probably wouldn't notice how and whether their conversation partners accommodate to them.
This poses difficulties on both evaluation and training, because it is not possible to put labels on accommodation measures without making a lot of assumptions.
...
This means that the manipulations are done on top of the existing \ac{tts} model, either by modifying the outputted waveform directly or by training a model that can take specific changes in feature description into consideration (see \cref{subsec:speech_manipulation}).
...
This work addresses each of those facets (see outline on pages \cpagerefrange{outline}{outline_end}).
...
Together, these properties results in a flexible, non-deterministic output derived from a defined core behavior (that might vary itself).
...
These technologies are closing the gaps between computer and human speech quality, and therefore help investigating vocal accommodation in \ac{hci} with system that can more closely simulate human performance.
Particularly, the overall quality and dynamic manipulation of modern \ac{tts} methods make is possible to change certain aspects in a system's voice (see \cref{subsec:speech_manipulation} for further details and examples).
...
Even tough this does not over a comprehensive coverage of all vocal changes that can be introduced in a conversation, the method of tracking and adapting the one feature can often be applied to others (similarly to the way features are defined in \cref{sec:parameters}).
...
\citet{Suzuki2003effects} introduces a system with pure prosodic vocal capabilities with no lexical content.
...
This is an important insight, since this is also not likely to happen in \ac{hhi} as each speaker has a different general speaking style and also change different if and when accommodation to an interlocutor (and see definitions in \cref{subsec:accommodation_levels}).
...
Tackling accommodation from a different direction, \citet{Ward2004automatic} developed a system that predicts the speech rate of phones operators based on their interlocutors' speech.
While the method was somewhat simplistic and was applied on monotonic conversations with predictable content, it nonetheless shows that some general tendencies can be found based on vocal behavior in a conversation.
As for implementation and integration, \citet{Levitan2016implementing} uses a modular way for adding accommodation capabilities to a \ac{sds} while separating the technical methods from the modeling details.
This approach is similar to the one used in this work.
Specifically, \cref{chap:computational_model,chap:statistical_model} discuss ways for modeling accommodation and define system behaviors while module presented in \cref{chap:convergence_module_for_sdss} shows how to integrate those into a system.
A major difference between this system and this work is the temporal and non-linear aspects of the accommodation process.
While the system in \citet{Levitan2016implementing} always directly converges towards the user at a defined rate (and till a perfect match), the modeling in this work aim to give a system an individual way to \emph{respond} to the user's behavior based on some core profile, as explained in \cref{subsec:accommodation_levels,fig:incr_gen_models}.
