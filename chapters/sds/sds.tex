...
\todo[inline]{the following sentence is from Weise's paper. write something along these line to connect one-sided human convergence to computer's side convergence}
%Once the fundamental role of entrainment in human interaction had been established and the first studies had proven that it affected human-computer interaction as well, researchers started developing systems that could actively entrain to human utterances rather than just being entrained to by their users.

\todo[inline]{embed the following sentences somewhere here}
...
Integrating such accommodative capabilities is challenging, both from the technical and design point of views, as discussed in \cref{subsec:suggested roadmap}.
According to \citet{Weise2017towards}, integrating such changes into computer systems will enhanced \ac{hci} and provide improved tools for studying accommodation also from the computers' side.
He also notes that dialogue systems with accommodation capabilities could offer an additional layer of interaction, namely responding to user's behavior. -- Either by observing user's behavior and use it as additional information for some analyses, or by actively making use of accommodation to encourage the user to speak in a specific way.
...
\todo[inline]{talk in detail about the challenge that there is no absolute correct answer regarding how and how much to converge (user dependent, etc.) and for the same reasons there is no gold standard to compare against. so need to find other criteria for evaluation. this is also true for a system that simulates accommodation capabilities (reference to chapter System)}
