...
This offers benefits to the users, like begin able to perform tasks while keeping their hands free, contrary to systems that requires textual input from a keyboard or haptic touch on a screen.
...
This makes the understanding and improving of social skills in \acp{sds} all the more important.
...
\Cref{sec:types_of_sdss} gives an overview of some application that use a \ac{sds} to communicate with users.
...
Each cycle starts and ends with speech signals, generated first by the user and then by the system (more sophisticated system can also take the initiative).
The content of the utterance, usually referred to as \emph{intent}, is then extracted to determine its objective.
Similarly, the system's speech output is based on a generated content that captures some intent.
The \enquote{brain} at the core of the cycle decides what intent is most suitable for the user's input.
This maybe done purely by learning from provided dialogue examples, with the help of external information or databases, based on hand-crafted rules, or some combination of those.
...
Although each component is a whole research area by itself, there are numerous open-source implementation that allow to quickly built a basic \ac{e2e} system and focus on a specific one.
...
Note that this is a basic typical architecture and each component may be extended or modify.
...
For the purpose of vocal accommodation, one benefit of use such modifiable toolkits is the ability to access the phoneme times.
...
Deciding on the best action can be achieved using a deterministic rule system for a small number of simple cases (for example a \ac{cnc} system, for example), but need more sophisticated models for more involve situations.
...
After deciding on the most appropriate response to the user, the system needs to conveys it in a human-understandable manner, namely words.
The process is reversed to \ac{nlg}, i.e., generating text based on a certain intent.
Depending on the user's input intent, the system may respond with simple acknowledgment statements, repetition and approval of information, or completely newly formed full sentences.
...
The last step the flow is converting the text provided by the \ac{nlg} component to speech signal and play it to the user.
...
In addition to being used as \ac{e2e} \ac{tts} piplines, these systems can also provide intermediate analysis outputs like phonetic transcriptions.
...
\Cref{tab:sds_types} compares between these two main categories.
...
\Acp{sds} can be utilized in various ways and be embedded in different types of system.
...
A \acl{pa} (\acs{pa}; frequently also \emph{intelligent \acl{pa}} or \emph{virtual \acl{pa}}\footnote{The terms \emph{virtual assistant} is widely used as well. However, it is avoided here, since it also refers to a different kind of occupation (\url{https://www.investopedia.com/terms/v/virtual-assistant.asp}).}) is a software-based program (sometimes embedded into a dedicated device (see \cref{subsec:smart_speakers}) that in some way fills the role of a human-being personal assistant.
...
Smart speakers (or \emph{intelligent speakers}) are small loudspeakers typically used by one to several users in a common household of working environment.
The speakers themselves merely offer audio transmission with some basic, hands-free operation.
The \enquote{smart} components comes from the software installed on it, which is usually some variety of a \ac{pa} (see \cref{subsec:personal_assistants}).
...
Due to their nature, chatbots can be utilized in a variety of ways, and are usually embedded into social robots, virtual agents, or smart speakers that offer one as a separate functionality.

An early example of a program considered a chatbot with a defined purpose is ELIZA \citep{Weizenbaum1966eliza}, which tried to imitate the role of therapist in a therapeutic session.
...
Nowadays, chatbots have already been used in various domains, such as education \citep{Benotti2014engaging, Kerly2007bringing}, elderly care \citep{Iio2020twin}, culture heritage \citep{Pilato2005expert}, healthcare \citep{Kowatsch2017text}, software development \citep{Lebeuf2017software}, and others \citep{Shawar2007chatbots}.

\subsection{Embodied agents and social robots}
\label{subsec:embodied_agents}

Embodied agents (sometimes also \emph{interface agent}) are communicative systems with some visual form.
Though the embodiment may be graphical only (see \cref{subsec:virtual_humans}, this term usually refer to systems that interact and communicate with the environment through some physical shape.
...
Similar to agents with physical embodiment (\cref{subsec:embodied_agents}), these conversational agents are capable  of multimodal communication.
...
Experiments have shown vocal accommodation effects in interactions with \acp{vh} with respect to feature like speech rate and \ac{f0} \citep{Gijssels2016speech, Staum2010virtually}.
