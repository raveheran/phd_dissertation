...
As speaking is an essential part of sales representatives' everyday work, controlling their voice is key for success.
In addition to the extent of the effects, it is also examined which of the speakers leads the accommodation process in succussful and failed calls.

\pagebreak

\acresetall

\section{Harnessing speech alignment in conversation intelligence}
\label{sec:conversation_intelligence}

Sales managers have always been impelled to be able to reason why some of their representatives (henceforth \emph{reps} or \emph{\acp{ae}}) consistently attain and even exceed their goals while others do not \citep{Kovac2017its}.
Yet, sales executives rely on data that are inherently flawed, as it is based on reports from sources like \ac{crm} systems.
Such systems contain only \enquote{dry} details about deals' stage, along with high-level numeric data and some, typically subjective, estimations regarding their potential in the following stages.
That leaves the executives in the dark regarding the happenings at the front lines and the small-scale, day-to-day conducts and modi operandi, which are critical for \acp{ae}' success.
As a result, the reasons for losing or winning a deal often remain a riddle, making sales seem more like art based on anecdotes rather than a scientifically explainable practice \citep{Yohn2016best, Martin2017six}.
...
Some phonetic features, such as \Acf{f0}, can be interpreted and explained by people with no phonetic training, like \acp{ae}.
...
For this reason, the analyses presented in this chapter focus on \ac{f0}, although other suprasegmental features showed similar effects.
To that end, a large-scale corpus of real-world sales conversations was collected (\cref{sec:dataset_calls}).
...
This method finds instances where coordinates of two time series occur close to each other within a certain radius in a phase-space continuum.only
...
It complements other types of human intelligence, like \emph{emotional intelligence} (c.f.\ \emph{Theory of Multiple Intelligences} \citep[][]{Gardner1983frames, Davis2011theory}), as the ensemble of conversational aspects in human communication \emph{beyond the surface words and shared information}.
...
Although \ac{c-iq} comprises many further aspects that are beyond the scope of this work, the overarching idea that communication quality is a defined, learnable skill that can be refined is highly relevant for vocal accommodation but has not been explored in previous work.
...
Although accommodation is not a traditional measure in \ac{c-iq}, there have been attempts to use it as an additional factor in conversation evaluations.
\citet{Glaser2016conversational} emphasizes the importance of the turn-level alignment between speakers in a business call.
Lack of alignment may result in a skeptic and even resisting behavior that might lower the success chances of the call \citep[see Table 2 in][]{Glaser2014conversational}.
...
These calls were all made by trained sales representatives and were aimed at high-stakes deals\footnote{In this case, around US\$\num100,000 each, as opposed to occasional mass calls to random people for selling small products where the stakes and risks are very small in comparison.
...
These criteria follow common success measure conventions of \ac{b2b} calls\footnote{The benchmarks for successful deals are much more elaborate in practice and usually consider an entire selling process that comprises a series of calls.
...
Splitting the turns also creates equal, consecutive, continuous time units in the conversations, which are more comparable, without introducing artificial boundaries by dividing it into a pre-defined number of parts based on some arbitrary criterion.
...
As explained in \Cref{subsec:limitations_of_did,fig:accommodation_types}, these methods are limited to the detection of local effects that evolve gradually across adjacent turns.
...
An overview of the \ac{crqa} method is given by \citet{Wallot2018analyzing}.
...
This is useful for describing different the types of accommodation described in \cref{subsec:variation_types}, like synchronicity and alignment.
...
This ability to not only detect latent accommodation but also determine its initiator on a fine-grained time scale enables the description and detection of more complex accommodation behavior, such as delayed synchrony (see \cref{fig:accommodation_types}).
...
Since multiple variables were compared, the Bonferroni correction \citep{Bonferroni1936teoria} was applied, so that the overall error rate across all variables is $\alpha = 0.05$.
...
\begin{table}[t]
	\caption[P-values and means comparison between \acs{crqa} in sucess and failed calls]
...
In the context of accommodation, this cross-correlation indicates which speaker was leading the change.
...
Interactivity and speech balance were measure for a superset of the dataset presented in \cref{sec:dataset_calls},  which consisted of more than \num{1000} calls.
...
Another possible explanation is that reps give up the lead when a call is on the verge of failure and instead let the prospects lead to give them a better feeling.
...
When \acfp{ae} lead, they tend to do so at an earlier stage than prospects, all the more so in successful calls.

These findings suggest that \acp{ae} do not necessarily always lead the conversation, but know how and when to exploit this technique, consciously or not, to improve their stance in a call.
