...
For example, the results of the study described in \cref{chap:shadowing_in_sung_music_and_human_computer_interaction} reveal a high degree of variation across participants.
...
In addition, not only the features which introduce meaningful difference are language- and culture-dependent, they might also differ based on the specific situation in which the interaction takes place.
For example, a segmental feature of a language where a phoneme can be realized in two alternative ways (as explained in \cref{subsubsec:target_features_HCIConv}) will probably be ignored by a speaker of a language where only one of these vowels exist in its repository.
...
This step corresponds the human internal, often unconscious, linguistic knowledge, and how it is used to decide which detected realizations are legitimate new instances that will be stored in memory.
...
For example, how prone is the speaker to be influenced by external others' speech and how easily should the change be triggered.
...
This is fitting for systems with which the user is expected to have long interactions and/or many interactions over an extended period of time.
...
First, it tells the \ac{asr} component which phoneme is associated with this phoneme, so that it is forwarded for further analysis.
...
The context is a regular expression against which the phoneme sequence retrieved by the \ac{asr} is matched\footnote{This requires an \ac{asr} engine that returns such a sequence in addition to textual output.
...
\begin{landscape}
	\begin{table}[tb]
		...
		\begin{tabulary}{\linewidth}{lLL}
			...
			& typically $n \in (0, 1) \subset \mathbb{R}$; 0 for ignoring the pool ; $> 1$ for over-weighting the pool value\\\addlinespace[0.2cm]
			...
			&  $n \in (0, 1] \subset \mathbb{R}$; 1 (\SI{100}{\percent}) for no limitation \\
			\bottomrule
		\end{tabulary}
